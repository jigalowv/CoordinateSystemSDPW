...
- [DistanceUtils](./src/Extensions/DistanceUtils.cs) - статичні методи для обчислення відстаней між точками у 2D та 3D просторах.
- [FloatingPointUtils](./src/Extensions/FloatingPointUtils.cs) - допоміжні методи для роботи з числами з плаваючою комою, наприклад, порівняння з врахуванням похибки.
- [Benchmark2D](./src/Benchmark/Benchmark2D.cs) - реалізація бенчмарку для 2D точок у декартовій та полярній системах координат.
- [Benchmark3D](./src/Benchmark/Benchmark3D.cs) - реалізація бенчмарку для 3D точок у декартовій та сферичній системах координат.
...
CartesianPoint2D: x=3.0000, y=4.0000
PolarPoint: radius=5.0000, angle=0.9273
Converted back to CartesianPoint2D: x=3.0000, y=4.0000
...
CartesianPoint3D: x=2.0000, y=3.0000, z=6.0000
SphericalPoint: radius=7.0000, azimuth=0.9828, polarAngle=0.7854
Converted back to CartesianPoint3D: x=2.0000, y=3.0000, z=6.0000
...
**2D Простір:** Обчислення відстаней у полярних координатах повільніше через наявність тригонометричних функцій. Використання декартових координат для масових обчислень більш ефективне.  

**3D Простір:** Декартові координати забезпечують найшвидші обчислення відстані/ Хордовий метод у сферичних координатах трохи повільніший. Дугова відстань - найповільніший метод через складні тригонометричні обчислення. Пояснення різниці сум відстаней: хорда вимірює пряму між точками, а дуга - довжину по поверхні сфери.

**Загальний висновок:** Було закріплено навички створення імутабельних моделей, перетворень координат та обчислення відстаней. Проведено порівняння продуктивності, яке показало вплив складності формул та тригонометричних обчислень на швидкодію.